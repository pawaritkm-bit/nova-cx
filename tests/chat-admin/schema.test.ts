...
describe("setMemberSchema — บทบาทที่ไม่ใช่นักบัญชี/หัวหน้า ห้ามผูกพนักงาน", () => {
  ...
  it("★ [L1] accountant ไม่มี employee_id → ไม่ผ่าน (ต้องผูกพนักงาน)", () => {
    const r = setMemberSchema.safeParse({ chat_member_id: UUID, member_kind: "accountant", employee_id: "" });
    expect(r.success).toBe(false);
    if (!r.success) expect(r.error.issues.some((i) => /เลือกพนักงาน/.test(i.message))).toBe(true);
  });
  it("★ [L1] lead ไม่มี employee_id → ไม่ผ่าน", () => {
    expect(setMemberSchema.safeParse({ chat_member_id: UUID, member_kind: "lead", employee_id: "" }).success).toBe(false);
  });
});
