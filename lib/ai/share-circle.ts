...
/**
 * ชื่อวง "คล้ายเลขบัตร/เบอร์/วันเกิด/รหัส" ไหม (garbage guard)
 *   = หลัง strip ตัวคั่น (เว้นวรรค/ขีด/สแลช/จุด) แล้วเหลือ 'ตัวเลขล้วน ≥ 9 หลัก'
 *   ครอบ: เลขบัตร 13 หลัก, เลขผู้เสียภาษี, เบอร์โทร 10 หลัก, วันเกิดยาว ๆ, รหัสตัวเลขยาว
 */
function looksLikeIdNumber(name: string): boolean {
  const digits = name.replace(/[\s.\-/]/g, "");
  return /^\d{9,}$/.test(digits);
}

/** normalize 1 วงดิบ → ParsedShareCircleEntry (คืน null ถ้าไม่มีเนื้อหาจริง/เป็น garbage) */
export function normalizeCircle(raw: unknown): ParsedShareCircleEntry | null {
  ...
  // มี "สัญญาณเงิน/วง" อย่างน้อย 1 ไหม (G/H/I/J/K/ต้น/สมาชิก)
  ...
  // ไม่มีชื่อวง + ไม่มีสัญญาณเงินเลย = ไม่มีเนื้อหาจริง → ทิ้ง
  if (!name && !hasMoneySignal) return null;

  // ★ garbage guard (บั๊ก P718): ชื่อวงเป็นเลขบัตร/เบอร์/วันเกิด/รหัส (ตัวเลขล้วน ≥9 หลัก)
  //   และไม่มีสัญญาณเงินใด ๆ → AI อ่านโปรไฟล์/เลขบัตรมาเป็นวงมั่ว → ทิ้ง
  if (name && !hasMoneySignal && looksLikeIdNumber(name)) return null;
...
const SYSTEM_PROMPT =
  ...
  "round_note = รอบเปีย/รอบการส่ง เป็นข้อความ (เช่น 'รายเดือน', 'ราย 10 วัน', 'ราย 15 วัน', 'รายสัปดาห์') " +
  "— เก็บข้อความตามที่เขียนในลิสต์, " +
  "member_count = จำนวนสมาชิกในวง เป็นจำนวนเต็ม (เช่น '21 คน'→21, '16คน'→16) — ★ ไม่ต้องแยกรายชื่อสมาชิก เอาแค่จำนวน, " +
  ...
  // ---- ★ ย้ำ: รอบเปีย + จำนวนสมาชิก ต้องพยายามกรอกให้ครบทุกวง ----
  "★★★ สำคัญ — 'ทุกวง' ต้องพยายามกรอก round_note (รอบเปีย) และ member_count (จำนวนสมาชิก) ให้ครบเสมอ " +
  "ถ้ามีข้อมูลนั้นอยู่ในลิสต์: ลิสต์วงแชร์ปกติแต่ละวง/บรรทัดมักระบุรอบเปียและจำนวนคนไว้อยู่แล้ว " +
  "(เช่น 'วงบิท รายเดือน 21 คน') → ต้องจับ round_note='รายเดือน' และ member_count=21 ให้ครบทุกวง อย่าปล่อยเป็น null. " +
  "รอบเปียอาจอยู่หัวลิสต์/หัวข้อรวม (ใช้กับทุกวงในลิสต์นั้น) หรือระบุรายวง — ถ้ามีรอบรวมให้ใช้กับทุกวงที่ไม่ได้ระบุรอบเอง. " +
  "ใส่ null สำหรับ round_note/member_count 'เฉพาะเมื่อในลิสต์ไม่มีข้อมูลนั้นจริง ๆ' เท่านั้น. " +
  "★ ช่องไหนไม่มี/ไม่ชัดในลิสต์ ให้ใส่ null (อย่าเดามั่ว โดยเฉพาะตัวเลขเงิน G/H/I/J/K — เว้นว่างดีกว่าใส่ผิด). " +
  "★ ตัวเลขทุกช่องเป็นตัวเลขล้วน ห้ามมีลูกน้ำคั่นหลักพัน (เช่น 100000 ไม่ใช่ 100,000). " +
  // ---- กันอ่าน "ข้อมูลที่ไม่ใช่วงแชร์" มาสร้างวงมั่ว (บั๊ก: อ่านโปรไฟล์/เลขบัตรมาเป็นวง) ----
  "★★ สำคัญมาก — สกัด 'เฉพาะลิสต์วงแชร์จริง' เท่านั้น: วงที่มีชื่อวง (มักขึ้นต้น 'วง...') " +
  "และ/หรือมีตัวเลขเงินของวง (รายได้ท้าว/ค่าบริหาร/ดอกเบี้ย/เงินต้น). " +
  "★★ ห้ามทำเป็นวงเด็ดขาด — ให้เพิกเฉย (ข้าม) ข้อมูลเหล่านี้ที่ปนมาในกลุ่มไลน์: " +
  "เลขบัตรประชาชน/เลขประจำตัวผู้เสียภาษี (เลข 13 หลัก), วันเกิด/วันที่ (เช่น 02112534, 02/11/1991), " +
  "เบอร์โทรศัพท์, รหัสผ่าน/รหัสอ้างอิง, ที่อยู่, ชื่อ-นามสกุลบุคคล, ข้อความแชตคุยทั่วไป, " +
  "รูปโปรไฟล์/ข้อความโปรไฟล์, ใบเสร็จ/ใบกำกับภาษี/สลิปโอนเงิน. " +
  "★★ ถ้าเนื้อหาที่ให้มา 'ไม่ใช่ลิสต์วงแชร์เลย' (เป็นโปรไฟล์/เลขบัตร/แชตทั่วไป/บิล) " +
  'ให้คืน {"circles":[]} — อย่าเดา อย่าสร้างวงมั่วจากตัวเลข/ข้อความที่ไม่ใช่วงแชร์. ' +
  "★★ ชื่อวง (circle_name) ต้องเป็น 'ชื่อวงแชร์' จริง ห้ามเอาเลขบัตร/เลขรหัส/วันเกิด/เบอร์โทรมาเป็นชื่อวง.";

const USER_PROMPT =
  "อ่านเนื้อหาต่อไปนี้ (ข้อความและ/หรือรูป) แล้วสกัด 'เฉพาะลิสต์วงแชร์จริง' เป็น JSON ตามรูปแบบ. " +
  "แยกให้ครบทุกวง. ถ้าไม่ใช่ลิสต์วงแชร์ (เป็นโปรไฟล์/เลขบัตร/แชตทั่วไป/บิล) ให้คืน circles ว่าง. " +
  "★ ทุกวงต้องพยายามกรอก round_note (รอบเปีย) + member_count (จำนวนสมาชิก) ให้ครบถ้ามีในลิสต์ — ใส่ null เฉพาะเมื่อไม่มีจริง. " +
  "จำไว้: ช่องเงินไหนไม่ชัดให้ null, ตัวเลขห้ามมีลูกน้ำหลักพัน, ห้ามเอาเลขบัตร/วันเกิดมาเป็นชื่อวง.";
