...
import { generateDueOccurrences, todayIsoThai } from "@/lib/accounting/recurring-invoice";
...
/**
 * POST/GET /api/cron/generate-recurring-invoice
 *   ใบแจ้งหนี้ลูกค้าแบบวนซ้ำ (wishlist ข้อ 4) — Vercel Cron รายวัน
 *   สแกนทุกเทมเพลตที่ `is_active=true` และ `next_run_date <= วันนี้` ของ "ทุก tenant" (service-role
 *   ไม่ผูก tenant เดียว — mirror app/api/cron/generate-recurring-je/route.ts) →
 *   claim (atomic RPC) + สร้าง occurrence (bill_entries entry_type='sale') เป็น draft เสมอ
 *
 ...
 *   เป็น error loop) — generateDueOccurrences เองก็ครอบ try/catch ต่อเทมเพลตอยู่แล้ว
...
async function handle(request: NextRequest) {
  ...
  try {
    ...
    logServerError("cron/generate-recurring-invoice", requestId, e);
