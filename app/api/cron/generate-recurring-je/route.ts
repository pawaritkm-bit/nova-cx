...
import { generateDueOccurrences, todayIsoThai } from "@/lib/accounting/recurring-journal";
...
/**
 * POST/GET /api/cron/generate-recurring-je
 *   Recurring Journal Entry — Vercel Cron รายวัน (เฟส 6 ส่วน R, docs/06 หมวด 0.4/0.8)
 *   สแกนทุกเทมเพลตที่ `is_active=true` และ `next_run_date <= วันนี้` ของ "ทุก tenant" (service-role
 *   ไม่ผูก tenant เดียว — mirror `redecideAllTenants` ของ app/api/cron/extract-bills/route.ts) →
 *   claim (atomic RPC) + สร้าง occurrence เป็น draft เสมอ (0.3 ห้าม auto-confirm)
 *
 * ความปลอดภัย (fail-closed): ไม่ตั้ง CRON_SECRET → ปิด endpoint (503, ไม่รัน) · auth ผิด → 401
 * ★ error ภายใน (เทมเพลตใบใดใบหนึ่งพัง/DB blip) → catch แล้วคืน 200 เสมอ (กัน Vercel Cron retry
 *   เป็น error loop) — generateDueOccurrences เองก็ครอบ try/catch ต่อเทมเพลตอยู่แล้ว (0.8)
 * ★ today = todayIsoThai() (server เท่านั้น) — ไม่มีทางรับจาก client/query param
 */

/** สแกนทุก tenant ที่มีเทมเพลต active ถึงกำหนดวันนี้ → generate ทีละ tenant (cron ไม่ผูก tenant เดียว) */
async function generateForAllTenants(
  ...
  for (const tenantId of tenantIds) {
    const r = await generateDueOccurrences(db, tenantId, today);
    scanned += r.scanned;
...
async function handle(request: NextRequest) {
  ...
  try {
    ...
    logServerError("cron/generate-recurring-je", requestId, e);
