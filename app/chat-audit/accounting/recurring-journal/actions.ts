...
/**
 * Server actions ของหน้า "รายการบันทึกซ้ำ" (/chat-audit/accounting/recurring-journal — เฟส 6 ส่วน R)
 *
 * flow ความปลอดภัย (ยึดมาตรฐานเดียวกับ journal-entry/actions.ts):
 ...
 *   3) validate ซ้ำฝั่ง server เสมอ (recurring-journal.ts::validateTemplateInput ผ่าน upsertTemplate)
 *   4) revalidatePath('/chat-audit/accounting/recurring-journal')
 *
 * ★ 0.3/0.4 ปุ่ม "สร้างตอนนี้" (generateNowAction) — บังคับ `today` = todayIsoThai() ฝั่ง server เสมอ
...
const PATH = "/chat-audit/accounting/recurring-journal";
...
export type RecurringSaveResult = { ok: boolean; message: string; id?: string };
...
/** บันทึกเทมเพลต (สร้างใหม่/แก้ไข) — ไม่สมดุล/รหัสบัญชีไม่อยู่ในผัง/frequency ไม่รู้จัก → ปฏิเสธ */
export async function saveTemplateAction(input: SaveTemplateInput): Promise<RecurringSaveResult> {
  try {
...
export async function deleteTemplateAction(id: string, customerId: string): Promise<RecurringSaveResult> {
  if (!isUuid(id) || !isUuid(customerId)) return { ok: false, message: "ไม่พบเทมเพลตที่เลือก" };
  try {
    const authed = await createClient();
    const service = createServiceRoleClient();
 ...
 *   ★ 0.4/T42 DoD: `today` = todayIsoThai() เสมอ (คำนวณฝั่ง server เท่านั้น) — ไม่รับจาก client
 *   เทมเพลตยังไม่ถึงรอบ (next_run_date > today) → claim ไม่ติด → คืน skipped (แจ้งผู้ใช้เฉย ๆ ไม่ throw)
 */
export async function generateNowAction(id: string, customerId: string): Promise<RecurringSaveResult> {
  if (!isUuid(id) || !isUuid(customerId)) return { ok: false, message: "ไม่พบเทมเพลตที่เลือก" };
  try {
    const authed = await createClient();
    const service = createServiceRoleClient();
    ...
    if (res.status === "generated") {
      return { ok: true, message: "สร้างรายการ (ร่าง) สำเร็จ — ไปยืนยันที่หน้าลงบันทึกบัญชีเอง" };
    }
