...
import { listAccountMap, listProductMap } from "@/lib/accounting/flowaccount-map";
import FlowAccountMapPanel from "../FlowAccountMapPanel";
...
/**
 * /chat-audit/accounting/flowaccount-map — "mapping ผังบัญชี/สินค้า nova-cx ↔ FlowAccount ต่อลูกค้า"
 *   เลือกลูกค้า (ในสโคป) → กรอกรหัสบัญชี/สินค้าฝั่ง FlowAccount ต่อรายการ (เฟส 5 ส่วน Q)
 *
 * ★ guard + scope เดียวกับหน้า opening/reports (resolveAccountingAccess) — นักบัญชีเห็นเฉพาะลูกค้าตัวเอง
 * ★ tenantId จาก session · ไม่ log ชื่อ/รหัสบัญชี/รหัสสินค้า
 */
export default async function FlowAccountMapPage({
  searchParams,
}: {
  ...
  if (!getSupabaseEnv()) {
    return (
      <ChatAuditFrame
        ...
        role={null}
        authed={false}
        title="Mapping FlowAccount"
        subtitle="ผังบัญชี/สินค้า ต่อลูกค้า"
      >
  ...
  // ผังบัญชี/สินค้าของ tenant — โหลดครั้งเดียว (active เท่านั้น) ส่งลง FlowAccountMapPanel
  ...
  const accountMap = validCustomerId ? await listAccountMap(service, access.tenantId, validCustomerId) : [];
  ...
  return (
    <ChatAuditFrame
      ...
      staffOnly={staffOnly}
      title="Mapping FlowAccount"
      subtitle="ผังบัญชี/สินค้า nova-cx ↔ รหัสฝั่ง FlowAccount ต่อลูกค้า"
    >
      <div className="dash-views">
        ...
        {customers.length === 0 ? (
          ...
          <div className="card">
            <p className="empty">เลือกลูกค้าด้านบนเพื่อจัดการ mapping FlowAccount</p>
          </div>
        ) : (
          <div className="card">
